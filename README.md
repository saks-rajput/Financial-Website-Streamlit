...
3. `ratio_engine.py` — plain, deterministic arithmetic (no AI) turning the
   ...
   ROE, free cash flow, a DuPont ROE breakdown, etc. - plus a separate
   bank-specific ratio set (ROE, ROA, efficiency ratio, loans-to-deposits,
   capital ratios) for financial institutions, where the standard ratio
   set doesn't apply.
...
The app is organized as: a persistent Headline Metrics summary at the
top, then four tabs for everything else - **Financial Ratios** (the key
ratios table, DuPont breakdown, and bank metrics if applicable),
**Executive Summary** (the AI's overall read plus strengths), **Risks**
(the AI's watch items / red flags, split out on its own), and **YoY
Trend** (the chart plus the AI's commentary on how things moved year to
year). The Executive Summary and Risks tabs don't make a second AI call -
they split the single narrative `generate_insights.py` returns by its
