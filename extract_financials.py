...
# Banks don't have current/non-current balance sheets, gross margin, or
# operating income - those concepts don't apply to how banks report. What
# they DO virtually always publish is a "Selected Financial Data" /
# "Financial Highlights" summary table with their own already-calculated
# ROE, ROA, efficiency ratio, loans-to-deposits ratio, and regulatory
# capital ratios. Pulling that table (when present) is far more reliable
# than trying to approximate bank-specific ratios from raw line items -
# these are the company's own reported figures, not an estimate.
BANK_METRICS_ANCHORS = [
    [r"return on (common )?equity", r"\bROE\b"],
    [r"return on assets", r"\bROA\b"],
    [r"efficiency ratio", r"overhead ratio"],
]
...
def build_financial_statements_excerpt(full_text: str) -> tuple:
    ...
    # Bank/financial-institution "Selected Financial Data" table - optional,
    # so its absence is NOT added to missing_statements. Most companies
    # simply don't have one, and that's expected, not an error. When it IS
    # found, appending it gives the AI a chance to fill in bank_metrics;
    # when it isn't, bank_metrics just stays null and nothing is lost.
    ...
    bank_metrics_section = find_real_statement(
        full_text, bank_metrics_headings, BANK_METRICS_ANCHORS,
        window=6000, anchor_window=5500, min_density=12,
    )
