...
ratio_engine.py, edgar_lookup.py, and generate_insights.py, all of which
are UI-framework agnostic. The interface is a coat of paint on a working
engine, not a rebuild.

Four things changed from the very first version of this file:
...
2. A defensive guard around `years` being empty: the original crashed the
   ...
   message instead.
3. Multiple years of history: a single annual report only has 2-3 years
   of figures baked in (an SEC rule, not a limit of the extraction), so
   the Year-over-Year Trend chart used to be pretty thin. Two ways to add
   more years, either or both: upload more than one annual report PDF
   (their years get merged, first file wins on any overlap), and/or enter
   a stock ticker so SEC EDGAR's own structured filing data can fill in
   additional years automatically, with no extra PDFs needed. Bank-
   specific metrics can't come from EDGAR (no standard XBRL tag for
   "efficiency ratio"), so EDGAR-sourced years correctly show those as
   Undisclosed rather than guessing.
4. Tabbed navigation: everything used to be one long scrolling page.
   Headline Metrics stays visible up top (like a persistent summary),
   and the deeper content - Financial Ratios, Executive Summary, Risks,
   and YoY Trend - now lives behind st.tabs(), so you click into the
   section you actually want instead of scrolling past all of them. The
   Risks and Executive Summary tabs don't make a second AI call - they
   split the one narrative generate_insights.py already returns by its
   "## Header" sections (see _parse_narrative_sections below).
...
def _parse_narrative_sections(narrative: str) -> dict:
    """generate_insights.py always writes the narrative as five '## Header'
    markdown sections (Executive Summary, Strengths, Watch Items /
    Possible Red Flags, Year-over-Year Trend, Overall Assessment). Splits
    it into {header: body} so different tabs can show different parts
    instead of one long wall of text repeated everywhere. If the AI
    reply doesn't follow the expected format for some reason, this just
    comes back with no matches - callers fall back to showing the full
    narrative rather than breaking."""
    sections = {}
    current_header = None
    current_lines = []
...
uploaded_files = st.file_uploader(
    "Upload one or more annual report PDFs",
    type=["pdf"],
    accept_multiple_files=True,
    help="Upload several years' reports at once (e.g. the 2020 and 2023 10-Ks) "
    "to widen the trend chart beyond what any single filing covers.",
)
ticker = st.text_input(
    "Optional: stock ticker to extend the trend chart further using SEC EDGAR",
    placeholder="e.g. AAPL",
    help="Fills in additional years using SEC's own structured filing data - "
    "only works for US-listed companies, and can't supply bank-specific "
    "metrics (those will show as Undisclosed for EDGAR-sourced years).",
).strip()

if uploaded_files:
    financials = {"years": {}}
    missing_statements_all = set()
    ...
    for uploaded_file in uploaded_files:
        ...
        with st.spinner(f"Asking Claude to extract the financial figures from {uploaded_file.name}..."):
            try:
                excerpt, missing_statements = build_financial_statements_excerpt(full_text)
                file_financials = extract_financials_from_text(excerpt)
            except Exception as e:
                st.error(f"Extraction failed for {uploaded_file.name}: {e}")
                st.stop()

        missing_statements_all.update(missing_statements)
        for year, year_data in file_financials.get("years", {}).items():
            if year not in financials["years"]:
                financials["years"][year] = year_data

    if missing_statements_all:
        st.warning(
            "Could not confidently find these statement(s) in at least one file: "
            + ", ".join(sorted(missing_statements_all))
            + ". Fields depending on them will show as missing below."
        )

    years = sorted(financials["years"].keys())
    if not years:
        st.error(
    ...
    year_source = {year: "PDF" for year in years}
    st.success(f"Extracted data for: {', '.join(years)} (from {len(uploaded_files)} file(s))")

    if ticker:
        with st.spinner(f"Pulling additional history for {ticker.upper()} from SEC EDGAR..."):
            edgar_years, edgar_error = fetch_annual_history(ticker)
        if edgar_error:
            st.warning(f"Could not extend history from SEC EDGAR: {edgar_error}")
        else:
            added_years = []
            for year, year_data in edgar_years.items():
                if year not in financials["years"]:
                    financials["years"][year] = year_data
                    year_source[year] = "SEC EDGAR"
                    added_years.append(year)
            if added_years:
                st.info(
                    f"Added {len(added_years)} more year(s) from SEC EDGAR: "
                    f"{', '.join(sorted(added_years))}. These weren't in your "
                    "uploaded PDF(s), so they use SEC's structured filing data "
                    "instead of AI extraction - bank-specific metrics aren't "
                    "available this way and will show as Undisclosed."
                )

    years = sorted(financials["years"].keys())

    pdf_bank_flags = [
        is_financial_institution(financials["years"][year])
        for year in years
        if year_source[year] == "PDF"
    ]
    company_is_bank = any(pdf_bank_flags)
    is_bank_by_year = {year: company_is_bank for year in years}
    ...
    if company_is_bank:
        st.info(
            ...
            "Metrics table in the Financial Ratios tab instead."
        )
    ...
    sections = _parse_narrative_sections(narrative) if narrative else {}

    tab_ratios, tab_summary, tab_risks, tab_trend = st.tabs(
        ["📊 Financial Ratios", "📝 Executive Summary", "⚠️ Risks", "📈 YoY Trend"]
    )
    ...
    with tab_ratios:
        ...
        if company_is_bank:
            ...
            st.caption(
                ...
                "estimates. Years sourced from SEC EDGAR instead of an "
                "uploaded PDF show as Undisclosed here (see note above)."
            )
            with st.container(border=True):
    ...
    with tab_summary:
        ...
        if narrative_error:
            st.error(f"Commentary generation failed: {narrative_error}")
        elif sections:
            st.markdown(sections.get("Executive Summary", narrative))
            strengths = sections.get("Strengths")
            if strengths:
                st.markdown("**Strengths**")
                st.markdown(strengths)
            overall = sections.get("Overall Assessment")
            if overall:
                st.markdown("**Overall Assessment**")
                st.markdown(overall)
        else:
            st.markdown(narrative or "No commentary was generated.")

    with tab_risks:
        st.subheader("Risks & Watch Items")
        if narrative_error:
            st.error(f"Commentary generation failed: {narrative_error}")
        else:
            risks_body = sections.get("Watch Items / Possible Red Flags")
            if risks_body:
                st.markdown(risks_body)
            else:
                st.info("No specific risk commentary was returned.")

    with tab_trend:
        st.subheader("Year-over-Year Trend")
        if len(years) > 1:
            st.caption("Years: " + ", ".join(f"{year} ({year_source[year]})" for year in years))
            trend_df = _trend_dataframe(ratios_by_year, bank_ratios_by_year, is_bank_by_year, years)
            st.line_chart(trend_df)
        else:
            st.info("Only one year of data - upload another report or add a ticker above for a real trend.")

        if not narrative_error:
            trend_narrative = sections.get("Year-over-Year Trend")
            if trend_narrative:
